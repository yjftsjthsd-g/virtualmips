...
/* Dump an instruction */
int mips64_dump_insn(char *buffer, size_t buf_size, size_t insn_name_size, m_va_t pc, mips_insn_t instruction)
{
   ...
   case 2:                     /* load/store instructions */
      ...
      snprintf(buffer, buf_size, "%8.8x  %s %s,%d(%s)",
               instruction, insn_name, mips64_gpr_reg_names[rt], offset, mips64_gpr_reg_names[base]);
      break;
   ...
   case 3:                     /* GPR[rd] = GPR[rs] op GPR[rt] */
      ...
      snprintf(buffer, buf_size, "%8.8x  %s %s,%s,%s",
               instruction, insn_name, mips64_gpr_reg_names[rd], mips64_gpr_reg_names[rs], mips64_gpr_reg_names[rt]);
      break;
   ...
   case 4:                     /* GPR[rd] = GPR[rt] op GPR[rs] */
      ...
      rd = bits(instruction, 11, 15);
      snprintf(buffer, buf_size, "%8.8x  %s %s,%s,%s",
               instruction, insn_name, mips64_gpr_reg_names[rd], mips64_gpr_reg_names[rt], mips64_gpr_reg_names[rs]);
      break;
   ...
   case 5:                     /* GPR[rt] = GPR[rs] op immediate (hex) */
      ...
      snprintf(buffer, buf_size, "%8.8x  %s %s,%s,0x%x",
               instruction, insn_name, mips64_gpr_reg_names[rt], mips64_gpr_reg_names[rs], imm);
      break;
   ...
   case 6:                     /* GPR[rt] = GPR[rs] op immediate (dec) */
      ...
      snprintf(buffer, buf_size, "%8.8x  %s %s,%s,%d",
               instruction, insn_name, mips64_gpr_reg_names[rt], mips64_gpr_reg_names[rs], (m_int16_t) imm);
      break;
   ...
   case 7:                     /* GPR[rd] = GPR[rt] op sa */
      ...
      snprintf(buffer, buf_size, "%8.8x  %s %s,%s,%d",
               instruction, insn_name, mips64_gpr_reg_names[rd], mips64_gpr_reg_names[rt], sa);
      break;
   ...
   case 8:                     /* Branch with: GPR[rs] / GPR[rt] / offset */
      ...
      snprintf(buffer, buf_size, "%8.8x  %s %s,%s,0x%" LL "x",
               instruction, insn_name, mips64_gpr_reg_names[rs], mips64_gpr_reg_names[rt], new_pc);
      break;
   ...
   case 9:                     /* Branch with: GPR[rs] / offset */
      ...
      offset = bits(instruction, 0, 15);
      new_pc = (pc + 4) + sign_extend(offset << 2, 18);
      snprintf(buffer, buf_size, "%8.8x  %s %s,0x%" LL "x", instruction, insn_name, mips64_gpr_reg_names[rs], new_pc);
      break;

   case 10:                    /* Branch with: offset */
      offset = bits(instruction, 0, 15);
      new_pc = (pc + 4) + sign_extend(offset << 2, 18);
      snprintf(buffer, buf_size, "%8.8x  %s 0x%" LL "x", instruction, insn_name, new_pc);
      break;
   ...
   case 13:                    /* op GPR[rs] */
      rs = bits(instruction, 21, 25);
      snprintf(buffer, buf_size, "%8.8x  %s %s", instruction, insn_name, mips64_gpr_reg_names[rs]);
      break;

   case 14:                    /* op GPR[rd] */
      rd = bits(instruction, 11, 15);
      snprintf(buffer, buf_size, "%8.8x  %s %s", instruction, insn_name, mips64_gpr_reg_names[rd]);
      break;
   ...
   case 15:                    /* op GPR[rd], GPR[rs] */
      ...
      snprintf(buffer, buf_size, "%8.8x  %s %s,%s",
               instruction, insn_name, mips64_gpr_reg_names[rd], mips64_gpr_reg_names[rs]);
      break;

   case 16:                    /* op GPR[rt], imm */
      rt = bits(instruction, 16, 20);
      imm = bits(instruction, 0, 15);
      snprintf(buffer, buf_size, "%8.8x  %s %s,0x%x", instruction, insn_name, mips64_gpr_reg_names[rt], imm);
      break;
   ...
   case 17:                    /* op GPR[rs], GPR[rt] */
      ...
      rt = bits(instruction, 16, 20);
      snprintf(buffer, buf_size, "%8.8x  %s %s,%s",
               instruction, insn_name, mips64_gpr_reg_names[rs], mips64_gpr_reg_names[rt]);
      break;
   ...
   case 18:                    /* op GPR[rt], CP0[rd] */
      ...
      rd = bits(instruction, 11, 15);
      snprintf(buffer, buf_size, "%8.8x  %s %s,%s",
               instruction, insn_name, mips64_gpr_reg_names[rt], mips64_cp0_reg_names[rd]);
      break;

   case 19:                    /* op GPR[rt], $rd */
      rt = bits(instruction, 16, 20);
      rd = bits(instruction, 11, 15);
      snprintf(buffer, buf_size, "%8.8x  %s %s,$%d", instruction, insn_name, mips64_gpr_reg_names[rt], rd);
      break;

   case 20:                    /* op GPR[rs], imm */
      rs = bits(instruction, 21, 25);
      imm = bits(instruction, 0, 15);
      snprintf(buffer, buf_size, "%8.8x  %s %s,0x%x", instruction, insn_name, mips64_gpr_reg_names[rs], imm);
      break;

   default:
      snprintf(buffer, buf_size, "%8.8x  %s (TO DEFINE - %d)", instruction, insn_name, tag->instr_type);
      return (-1);
   }
...
void mips64_dump_insn_block(cpu_mips_t * cpu, m_va_t pc, u_int count, size_t insn_name_size)
{
...
static forced_inline int mips64_exec_memop(cpu_mips_t * cpu, int memop, m_va_t vaddr, u_int dst_reg, int keep_ll_bit)
{
   mips_memop_fn fn;
...
/*m_reg_t data;
	...
	haddr = mips_mts32_access1(cpu,pc,MIPS_MEMOP_LW,4,MTS_READ,&data,&exc,&has_set_value,0);
	if (exc)
		return exc;
	if ((haddr==NULL)&&(has_set_value==FALSE))  
	{ 
		//bad_memory_access(cpu,pc);
		ASSERT(0,"pc %x\n",pc);
	}

	if (likely(has_set_value==FALSE)) data = vmtoh32(*(m_uint32_t *)haddr);
	if (likely(!exc)) *insn = data;
