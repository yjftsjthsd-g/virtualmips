...
void dma_non_descriptor_trans(cpu_mips_t *cpu,struct jz4740_dma_data *d,int channel)
{
  ...
  physmem_dma_transfer(cpu->vm,jz4740_dma_table[DMAC_DSAR(channel)/4],
                          ...
                          get_dma_unit_size(channel,jz4740_dma_table[DMAC_DCMD(channel)/4])*jz4740_dma_table[DMAC_DTCR(channel)/4]);
...
physmem_dma_transfer(cpu->vm,desc->dsadr,
                                                        desc->dtadr,
                                                        (desc->ddadr&0xffffff)*get_dma_unit_size(channel,desc->dcmd));
...
{
   /*DESCRIPTOR trans*/
  if ((jz4740_dma_table[DMAC_DMACR/4]&DMAC_DMACR_DMAE)&&(jz4740_dma_table[DMAC_DCCSR(channel)/4]&DMAC_DCCSR_NDES))
    {
