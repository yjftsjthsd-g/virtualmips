...
static void dev_cs8900_gen_interrupt(struct cs8900_data *d)
{
	...
	if ((*(m_uint16_t*)(ram_base+PP_BusCTL) )&(0x8000))
	{
		...
		cpu_log12(current_cpu,"","cs8900 irq\n");
	}
}
...
static inline int cs8900_handle_len(struct cs8900_data *d,
                                           m_uint8_t *pkt,ssize_t pkt_len)
{
	/*we do not check CRC !!!!*/
 	...
 	m_uint8_t * ram_base;
	ram_base = (m_uint8_t *)(&(d->internal_ram[0]));
   cpu_log13(current_cpu, "", "cs8900_handle_len  pkt_len %x \n",pkt_len);
...
static int dev_cs8900_receive_pkt(struct cs8900_data *d,
	...
	*(m_uint16_t*)(ram_base+PP_RxStatus) |= RxOKA;
  	if ((*(m_uint16_t*)(ram_base+PP_RxCFG))&RxOKiE)
	{
		...
		*(m_uint16_t*)(ram_base+PP_ISQ) |=RxEvent;
		cpu_log13(current_cpu,"","RX PPIS *(m_uint16_t*)(ram_base+PP_ISQ) %x \n",*(m_uint16_t*)(ram_base+PP_ISQ) );
...
static int dev_cs8900_rx(netio_desc_t *nio,
                                      u_char *pkt,ssize_t pkt_len,
...
static int dev_cs8900_tx(struct cs8900_data *d)
{
	...
	cpu_log13(current_cpu, "", "dev_cs8900_tx \n");
	...
	if ((*(m_uint16_t*)(ram_base+PP_LineCTL))&SerTxON)
		{
			...
			*(m_uint16_t*)(ram_base+PP_TxLength) = send_len;
			cpu_log13(current_cpu, "", "netio_send \n");
			...
			*(m_uint16_t*)(ram_base+PP_TxEvent) = TxOK | 0x8;  /*is = not |.  all other bits must be cleared*/
			if ((*(m_uint16_t*)(ram_base+PP_TxCFG))&TxOKiE)
			{
				...
				*(m_uint16_t*)(ram_base+PP_ISQ) |=TxEvent;
				cpu_log13(current_cpu,"","TX PPIS *(m_uint16_t*)(ram_base+PP_ISQ) \n",*(m_uint16_t*)(ram_base+PP_ISQ) );
				dev_cs8900_gen_interrupt(d);
			}
...
{
	//cpu_log14(current_cpu,"","cs8900a_rx_timeout %d\n",cs8900a_rx_timeout);
...
void dev_cs8900_cb(void *opaque)
{
	...
	cpu_log13(current_cpu, "", "dev_cs8900_cb \n");
	...
	case PP_RT_DATA1+1:
		...
		else if(op_type==MTS_WRITE)
		{
			...
			*has_set_value=TRUE;
			...
			cpu_log13(cpu, "", "d->tx_send_index %x \n",d->tx_send_index);
...
static int dev_cs8900_rxlistener(netio_desc_t *nio,
                                      u_char *pkt,ssize_t pkt_len,
                                      struct cs8900_data *d)
{
	if (d->want_rx==0)
...
static int dev_cs8900_tx(struct cs8900_data *d)
{
	...
	if ((*(m_uint16_t*)(ram_base+PP_LineCTL))&SerTxON)
		...
		d->want_tx=0; /*clear want_tx so that time will not generate interrupt*/
...
void dev_cs8900_active_timer(struct cs8900_data *d)
{
	vp_mod_timer(d->cs8900_timer, vp_get_clock(rt_clock)+1);
}
void dev_cs8900_unactive_timer(struct cs8900_data *d)
{
	vp_del_timer(d->cs8900_timer);
...
#define VALIDE_CS8900_OPERATION 1
static void *dev_cs8900_access(cpu_mips_t *cpu,struct vdevice *dev,
		m_uint32_t offset,u_int op_size,u_int op_type,
		m_uint32_t *data,m_uint8_t *has_set_value)
{
...
{   
   ...
   d->nio = nio;
   nio->can_recv=1;
   netio_rxl_add(nio,(netio_rx_handler_t)dev_cs8900_rxlistener,d,NULL);
   return(0);
}
