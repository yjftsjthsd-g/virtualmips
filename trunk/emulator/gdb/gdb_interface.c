...
int
delete_breakpoint(vm_instance_t *vm,m_uint32_t addr, int len)
{
...
alloc_breakpoint(vm_instance_t *vm,m_uint32_t addr, char *save,  int len)
{
...
int
insert_breakpoint(vm_instance_t *vm,virtual_breakpoint_t *new)
{
	virtual_breakpoint_t *tmp;
...
#define WORD_SZ  4
#define BYTE_SZ   1
int debug_read_memory(cpu_mips_t *cpu,m_uint32_t vaddr,
...
Simdebug_result
Simdebug_run(vm_instance_t *vm, int sig)
{
	static char cmd_buf[BUFSIZ+1];
	char *b;
	int i=0;
