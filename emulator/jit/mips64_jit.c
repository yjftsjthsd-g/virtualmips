...
int mips64_jit_init(cpu_mips_t * cpu)
{
    ...
    cpu->exec_page_area = mmap(NULL, cpu->exec_page_area_size,
                               PROT_EXEC | PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, (off_t) 0);
    ...
    printf("CPU%u: carved JIT exec zone of %lu Mb into %lu pages of %u Kb.\n",
           cpu->id,
           (u_long) (cpu->exec_page_area_size / 1048576), (u_long) cpu->exec_page_count, MIPS_JIT_BUFSIZE / 1024);
    return (0);
}
...
int mips64_jit_fetch_and_emit(cpu_mips_t * cpu, mips64_jit_tcb_t * block, int delay_slot)
{
    mips_insn_t code;
    register uint op;
...
/*set is_in_bdslot*/
    if ((delay_slot == 1) || (delay_slot == 2))
        x86_mov_membase_imm(block->jit_ptr, X86_EDI, OFFSET(cpu_mips_t, is_in_bdslot), 0x1, 4);

    mips_jit[op].emit_func(cpu, block, code);
/*clear is_in_bdslot*/
    if ((delay_slot == 1) || (delay_slot == 2))
        x86_mov_membase_imm(block->jit_ptr, X86_EDI, OFFSET(cpu_mips_t, is_in_bdslot), 0x0, 4);
    if (delay_slot == 1)
        block->mips_trans_pos++;
...
int mips64_jit_tcb_record_patch(mips64_jit_tcb_t * block, u_char * jit_ptr, m_va_t vaddr)
{
...
/* Apply all patches */
static int mips64_jit_tcb_apply_patches(cpu_mips_t * cpu, mips64_jit_tcb_t * block)
{
...
static int mips64_jit_tcb_adjust_buffer(cpu_mips_t * cpu, mips64_jit_tcb_t * block)
{
...
void mips64_jit_tcb_free(cpu_mips_t * cpu, mips64_jit_tcb_t * block, int list_removal)
{
    int i;
...
static mips64_jit_tcb_t *mips64_jit_tcb_create(cpu_mips_t * cpu, m_va_t vaddr)
{
...
static inline mips64_jit_tcb_t * mips64_jit_tcb_compile(cpu_mips_t * cpu, m_va_t vaddr)
{
...
static forced_inline void mips64_jit_tcb_run(cpu_mips_t * cpu, mips64_jit_tcb_t * block)
{
