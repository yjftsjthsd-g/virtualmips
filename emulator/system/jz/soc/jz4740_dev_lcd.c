		...
		case LCD_CTRL:
			if (op_type==MTS_WRITE)
			{
				...
				else
				{
					...
					/*INTERRUPT*/
					if ((jz4740_lcd_table[LCD_CTRL/4]&LCD_CTRL_QDM)==0x0)
					{
				...
				if (*data&LCD_CTRL_DIS)
				{
					...
					/*INTERRUPT*/
					if ((jz4740_lcd_table[LCD_CTRL/4]&LCD_CTRL_LDDM)==0x0)
					{
						current_cpu->vm->set_irq(current_cpu->vm,IRQ_LCD);
					}
...
void dev_jz4740_lcd_cb(void *opaque)
{
   struct jz4740_lcd_data *d=opaque;
	...
	//cpu_log15(current_cpu,"","jz4740_lcd_table[LCD_DA0/4] %x  \n",jz4740_lcd_table[LCD_DA0/4]);
	lcda0_desc =(struct jz_fb_dma_descriptor *) physmem_get_hptr(current_cpu->vm,jz4740_lcd_table[LCD_DA0/4],0,MTS_READ,&dummy);
	//cpu_log15(current_cpu,"","src %x dst %x  \n",lcda0_desc->fsadr,lcda0_desc->fdadr);
	...
	ASSERT(
			(((jz4740_lcd_table[LCD_CTRL/4]&LCD_CTRL_BPP_MASK)==LCD_CTRL_BPP_16)	
			||((jz4740_lcd_table[LCD_CTRL/4]&LCD_CTRL_BPP_MASK)==LCD_CTRL_BPP_18_24)),"Only 15/16/18/24 bpp supproted\n");
...
int dev_jz4740_lcd_init(vm_instance_t *vm,char *name,m_pa_t paddr,m_uint32_t len)
{
   ...
   d->ds = malloc(sizeof(*d->ds));
	...
	if ( dev_jz4740_ts_init(vm,"JZ4740 TS",JZ4740_TS_BASE,JZ4740_TS_SIZE)==-1)
		goto err_dev_create;
